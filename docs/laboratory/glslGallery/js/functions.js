...
setupClickDLButton = function(){

	//download btn
	$('.download-button').on('touchstart mousedown',function(){
	  $('.download-button').addClass('button-click');
	});
	$('.download-button').on('touchend mouseup mouseout touchleave',function(){
	  $('.download-button').removeClass('button-click');
	});
...
setupMainSpring = function() {
	...
	mainSpring.addListener({
		onSpringActivate:function(spring){
   		   ...
   		   $('.download-button').addClass('button-moving');
	...
	mainSpring.addListener({
		onSpringAtRest: function(spring){
			...
			$('.download-button').removeClass('button-moving');
		}
	})
}
...
layoutCaptions = function() {
	...
	$(".caption").each(function(i, val) {
		...
		var leftPadding = parseInt($(val).css("padding-left"), 24);	
	...
	$("#slides li canvas").each(function(i, val) {

		if(windowWidth > windowHeight){

			var mScaleX = windowHeight/1147*0.78
			var mScaleY = windowHeight/1147*0.78325

			val.style['transform'] = 'scaleX('+mScaleX+') scaleY('+mScaleY+')'

		}
		else{

			var mScaleX = windowWidth/1147*0.825
			var mScaleY = windowWidth/1147*0.83
...
setupTabPressedStates = function() {
	...
	downSpring.addListener({
		onSpringUpdate: function (spring) {
			var progress = spring.getCurrentValue();
			var scale = transitionForProgressInRange(progress,1.0,0.72);
...
selectTabIndex = function(i, animated) {
  ...
  $("#download-link").attr("href", hrefs[i]);
  ...
  //控制播放
  glslCanvases.forEach((sandbox,index)=>{
  	if(i!=index) {
  		sandbox.pause()
  	} else {
  		sandbox.play()
